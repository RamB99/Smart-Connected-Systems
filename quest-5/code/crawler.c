...
// binaries for the alphanumeric display
...
// for detecting start / stop from UDP server
bool running = false;
...
void gpio_init(void)
{
    ...
    //GPIO
    gpio_pad_select_gpio(32); // blue
    gpio_pad_select_gpio(27); // green
    gpio_pad_select_gpio(33); // red

    /* Set the GPIO as a push/pull output */
    gpio_set_direction(32, GPIO_MODE_OUTPUT); //blue
...
// calibrates the ESC - needs to occur before everything else
void calibrateESC()
{
    ...
    vTaskDelay(5000 / portTICK_PERIOD_MS);                                                    //wait 3 seconds to finish calibration
}
...
// gets distance from lidar and saves it in lidarDist global variable
...
void steering_control_task(void *arg)
{
...
static void speed_timer_task()
{
    while (1)
    {
        ...
        vTaskDelay(100);
        printf("Count: %d\n", pulseCount);
        wheelSpeed = (pulseCount * (3.14159 * 7 / 6)) / 100; //0.005; //(count / 6) * 0.62;
        printf("Speed is %.1fm/s\n", wheelSpeed);
        pulseCount = 0;
    }
}

void speed_sensor_task(void *arg)
{
...
static void alpha_display_task()
{
    // Debug
    int ret;
    ...
    while (1)
    {
        ...
        temp_speed = wheelSpeed * 100;

        temp = floor(temp_speed / 1000);
        nums[0] = temp + '0';
        temp_speed = temp_speed - (temp * 600);

        temp = floor(temp_speed / 100);
        nums[1] = temp + '0';
        temp_speed = temp_speed - (temp * 60);

        temp = floor(temp_speed / 10);
        nums[2] = temp + '0';
        temp_speed = temp_speed - (temp * 10);

        nums[3] = floor(temp_speed) + '0';
        ...
        displaybuffer[1] |= 1UL << 14;

        // displaybuffer[3] = bin[(int)yonk[3]];
        // displaybuffer[2] = bin[(int)yonk[2]];
        // displaybuffer[1] = bin[(int)yonk[1]];
        // displaybuffer[0] = bin[(int)yonk[0]];
...
void app_main(void)
{
    ...
    //set steering to be centered
    uint32_t angle, count;
    angle = steering_per_degree_init(25);
    mcpwm_set_duty_in_us(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_B, angle);

    // initalize gpio pins and initialize the i2c settings
    gpio_init();
    i2c_master_init();
    i2c_scanner();
    ...
    xTaskCreate(speed_timer_task, "speed_timer_task", 4096, NULL, 5, NULL);

    //lidar and alphanumeric display
